...
class CleanUpDatedUsers extends Command
{
    ...
    {
        $this->warn("Now Running [olduser:clear]");
        ...
        try {
            ...
            foreach ($databaseArray as $site) {
                ...
                \DB::table('login')
                    ->whereDate('last_login_time', '<=', \Carbon\Carbon::now()->modify('-3 months')->toDateTimeString())
                    ->orWhereNull('last_login_time')
                    ->delete();
            } // foreach

            \Log::channel('dbquerys')->info('---------------------------清理久未登入人員結束--------------------------');
            $this->info("[olduser:clear] Command executed successfully!");
        } catch (Exception $e) {
