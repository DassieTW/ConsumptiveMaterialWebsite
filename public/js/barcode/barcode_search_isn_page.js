function appenSVg() {
  ...
  $(".printNum").on("input", function (e) {
    ...
    $.grep(isnArray, function (isn, index) {
      ...
      if (
        isn === myArr[0] &&
        isnName[index] === myArr[1] &&
        isnSpec[index] === myArr[2]
      ) {
        index2 = index;
        return true;
      } // if
...
function UpdateTempField() {
  ...
  for (let ty = 0; ty < isnCount; ty++) {
    $("#tableHead").after(
      $(
        '<tr class="barcodePreview align-items-center" id="' +
          isnArray[ty] +
          "__" +
          isnName[ty] +
          "__" +
          ...
          "__" +
          isnSpec[ty] +
          '" class="printNum" name="printNum" style="width: 6ch;"value="' +
          isnSepCount[ty] +
          '">' +
          "</td></tr>"
      )
...
$(function () {
      ...
      success: function (response) {
        ...
        for (let a = 0; a < isnSheet.length; a++) {
          for (let b = a + 1; b < isnSheet.length; b++) {
            if (
              isnSheet[a] === isnSheet[b] &&
              nameSheet[a] === nameSheet[b] &&
  ...
  $("#printBtn").on("click", function (e) {
    // print all barcodes in the temp field
    e.preventDefault();
    $.ajaxSetup({
      headers: {
        "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
