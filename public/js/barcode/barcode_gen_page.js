...
function UpdateTempField() {
  ...
  for (let ty = 0; ty < isnCount; ty++) {
    $("#tableHead").after(
      $(
        '<tr class="barcodePreview isn align-items-center" id="' +
          isnArray[ty] +
          "__" +
          isnName[ty] +
          '">' +
          '<td class="col col-auto align-items-center px-0 m-0"><a class="deleteBtn" id="' +
          isnArray[ty] +
          "__" +
          isnName[ty] +
          '" value="' +
          ...
          '<input inputmode="numeric" type="number" min="0" id="' +
          isnArray[ty] +
          "__" +
          isnName[ty] +
          '" class="printNum" name="printNum" style="width: 8ch;"value="' +
          isnSepCount[ty] +
          '">' +
          "</td></tr>"
      )
  ...
  for (let ty = 0; ty < locCount; ty++) {
    $("#tableHead2").after(
      $(
        '<tr class="barcodePreview loc align-items-center" id="' +
          locArray[ty] +
          '">' +
          '<td class="col col-auto align-items-center px-0 m-0"><a class="deleteBtn" id="' +
          locArray[ty] +
          ...
          '<input inputmode="numeric" type="number" min="0" id="' +
          locArray[ty] +
          '" class="printNum2" name="printNum2" style="width: 8ch;"value="' +
          locSepCount[ty] +
          '">' +
          "</td></tr>"
      )
    ...
    success: function (response) {
      ...
      if (sessionStorage.getItem("isnArray") !== null) {
        for (let a = 0; a < isnSheet.length; a++) {
          ...
          if (key1 !== -1) {
            // if theres duplicate in upload file and/or the same as the one gen in website
            isnSepCountResult[key1] = isnSepCountResult[key1] + 1;
          } // if
          else {
            // the upload file has a new entry
          ...
          sessionStorage.setItem(
            "isnSepCount",
            JSON.stringify(isnSepCountResult)
          );
        } // for
      } // if
      ...
      if (sessionStorage.getItem("locArray") !== null) {
        for (let a = 0; a < locSheet.length; a++) {
          ...
          sessionStorage.setItem(
            "locSepCount",
            JSON.stringify(locSepCountResult)
          );
        } // for
      } // if
...
$(function () {
  (function () {
    ...
    if (!sessionStorage.hasOwnProperty("allMats")) {
      ...
      $.ajax({
        // get all isn and pName for bringing out pName if it existed in database
        type: "post",
    ...
    if (
      sessionStorage.hasOwnProperty("locCount") ||
      sessionStorage.hasOwnProperty("isnCount")
    ) {
      notyf.success({
        message: Lang.get("barcodeGenerator.temp_save_success"),
        duration: 5000, //miliseconds, use 0 for infinite duration
  ...
  $("#isnForm").on("input", function (e) {
    if ($("#barcode2").val().length === 12) {
      // when it fits the the 12 digits isn
      ...
      if (i === AllMatsData.length) {
        // isn not found in database
        $("#pName").focus();
      } // if
    } // if
  }); // on isnForm input
      ...
      success: function (data) {
        if (toSess === "true") {
          if (isIsn === "false" && sessionStorage.hasOwnProperty("locCount")) {
            ...
            var keyy = $.inArray(
              barcode3,
              JSON.parse(sessionStorage.getItem("locArray"))
            );
            if (keyy !== -1) {
        ...
        if (err.status === 422) {
          ...
          if (err.responseJSON.message === "Invalid parameters.") {
            ...
            $("#batchUp").after(
              $(
                '<span class="col col-auto invalid-feedback p-0 m-0" role="alert"><strong>' +
                  Lang.get("fileUploadErrors.Invalid_parameters") +
                  "</strong></span>"
              )
            );
          } // if
            ...
            $("#batchUp").after(
              $(
                '<span class="col col-auto invalid-feedback p-0 m-0" role="alert"><strong>' +
                  Lang.get("fileUploadErrors.No_file_sent") +
                  "</strong></span>"
              )
            );
          } // else if
            ...
            $("#batchUp").after(
              $(
                '<span class="col col-auto invalid-feedback p-0 m-0" role="alert"><strong>' +
                  Lang.get("fileUploadErrors.Exceeded_filesize_limit") +
                  "</strong></span>"
              )
            );
          } // else if
            ...
            $("#batchUp").after(
              $(
                '<span class="col col-auto invalid-feedback p-0 m-0" role="alert"><strong>' +
                  Lang.get("fileUploadErrors.Unknown_errors") +
                  "</strong></span>"
              )
            );
          } // else if
            ...
            $("#batchUp").after(
              $(
                '<span class="col col-auto invalid-feedback p-0 m-0" role="alert"><strong>' +
                  Lang.get("fileUploadErrors.Invalid_file_format") +
                  "</strong></span>"
              )
            );
          } // else if
          else if (
            err.responseJSON.message === "Failed to move uploaded file."
          ) {
            $("#batchUp").addClass("is-invalid");
            $("#batchUp").after(
              $(
                '<span class="col col-auto invalid-feedback p-0 m-0" role="alert"><strong>' +
                  Lang.get("fileUploadErrors.Failed_to_move_uploaded_file") +
                  "</strong></span>"
              )
            );
          } // else if
          else {
            // some weird err that im unaware of
            $("#batchUp").addClass("is-invalid");
            console.log(
              "some weird error when processing uploaded file : " +
  ...
  $("#printBtn").on("click", function (e) {
    ...
    if (
      ...
      $.ajax({
        ...
        cache: false,
        data: {
          isnArray: sendingISNArray,
          isnNameArray: sendingISNNameArray,
          isnSepCount: sendingISNSepCount,
          locArray: sendingLocArray,
          locSepCount: sendingLocSepCount,
          sID: sID,
        },
        dataType: "json", // let's set the expected response format
        beforeSend: function () {
        ...
        error: function (err) {
          ...
          notyf.error({
            message:
              Lang.get("barcodeGenerator.temp_save_error") +
              Lang.get("barcodeGenerator.print"),
            duration: 5000, //miliseconds, use 0 for infinite duration
            ripple: true,
            dismissible: true,
            position: {
