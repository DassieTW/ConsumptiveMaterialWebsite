$(function () {
  ...
  $(".sortBtn").on("click", function (e) {
    ...
    if ($(this).hasClass("active")) {
      ...
      if ($(this).attr("name") === "sortLocBtn") {
        ...
        if (
          $("#sortISNBtn").hasClass("active") &&
          $("#sortISNBtn").hasClass("sortUp")
        ) {
          sortTable("asc", "isnTable");
        } // if
        else if (
          $("#sortISNBtn").hasClass("active") &&
          $("#sortISNBtn").hasClass("sortDw")
        ) {
        ...
        $(this).attr("name") === "sortISNBtn" &&
        $(this).hasClass("sortUp")
      ) {
        sortTable("asc", "isnTable");
      } // else if
      else if (
        $(this).attr("name") === "sortISNBtn" &&
        $(this).hasClass("sortDw")
      ) {
        sortTable("dec", "isnTable");
      } // else if
    } else {
  ...
  (function () {
    ...
    if (document.getElementById("toggle-state").checked) {
      ...
      document
        ...
        .setAttribute("placeholder", Lang.get("checkInvLang.isn"));
    } // if
    else {
      $("#toggle-state-text").text(Lang.get("checkInvLang.loc_short"));
      document
        .getElementById("texBox")
        .setAttribute("placeholder", Lang.get("checkInvLang.loc_short"));
    } // else
  ...
  $(function () {
    ...
    function cb(start, end) {
      $("#reportrange span").html(
        start.format(moment().localeData().longDateFormat("L")) +
          " ～ " +
          end.format(moment().localeData().longDateFormat("L"))
      );
    ...
    json_rangeObj[past_three_months] = [
      moment().subtract(3, "month").startOf("month"),
      moment().subtract(0, "month").endOf("month"),
    ];
    json_rangeObj[all_history_records] = [
      moment().subtract(99, "year").startOf("year"),
      moment().subtract(0, "month").endOf("month"),
    ];
  ...
  var quickSort = function (data, data2, left, right, dir) {
    // data2 is only for moving row corresponding to data
    if (left < right) {
  ...
  function sortTable(dir, tableClass) {
    $("." + tableClass).each(function (index, obj) {
      ...
      if (tableClass === "locTable") {
        // if it is a Loc Table
    ...
    if (filterType === "filter_checked") {
      ...
      $(".isnTable").each(function (index, obj) {
        ...
        $.each(rows, function (i, item) {
          ...
          if (
            resultArr[1] !== "N/A" &&
            parseInt(resultArr[0], 10) === parseInt(resultArr[1], 10) &&
  ...
  function collapseByLoc() {
    ...
    for (let b = 0; b < keys.length; b++) {
      ...
      if (!keys[b].includes("_byLoc")) {
        var serialNumDataRow = $("<tr>", {
          "data-bs-toggle": "collapse",
          "data-bs-target": "#sheet" + b,
          "aria-expanded": "false",
        }); // create an elemet by jquery
        ...
        let thisSeasonStartingDate = moment()
          .subtract(3, "months")
          .startOf("month");
        if (dateCreated.isBefore(thisSeasonStartingDate)) {
          // then this is a old record that shoudnt be changed
          serialNumDataRow.append(
            '<td style="color: gray;">' +
              keys[b] +
              ' <a href="/checking?sheet=' +
              keys[b] +
              '"><i class="bi bi-clipboard-plus" style="color: blue; font-size: 1.2em;"></i></a>' +
              "</td>"
          );
          serialNumDataRow.append(
            '<td style="color: gray;">' + sheetCreators[sheetCount] + "</td>"
          );
          serialNumDataRow.append(
            '<td style="color: gray;">' +
              serialSheetsObj[keys[b]][0].created_at +
              "</td>"
          );
        } // if
        else {
          ...
          serialNumDataRow.append(
            "<td>" + serialSheetsObj[keys[b]][0].created_at + "</td>"
          );
        } // else

        serialNumDataRow.append(
          '<td><a class="collapseBtn" disabled style="color: grey;"><i class="bi bi-chevron-down"></i></a></td>'
        );
        var detailDataRowWithCollapse = $("<tr>", {});
        var detailTD = $("<td>", { colspan: "12", class: "p-0 m-0" });
        var locCollapseDiv = $("<div>", {
        ...
        var locTable = $("<table>", {
          class:
            "table table-primary table-hover align-items-center table-responsive m-0 p-0 locTable",
        });
        var thead = $("<thead>", {
          class: "table table-primary table-hover m-0 p-0",
        });

        var tbody = $("<tbody>", {});
        for (let c = 0; c < serialSheetsObj[keys[b] + "_byLoc"].length; c++) {
          // loop thru byLoc arrays
          var locName = Object.keys(serialSheetsObj[keys[b] + "_byLoc"][c])[0];
          var dataTR = $("<tr>", {
            class: "locRows",
            "data-bs-toggle": "collapse",
            "data-bs-target":
              "#locData" + keys[b].substring(0, keys[b].length - 9) + "_" + c,
            "aria-expanded": "false",
          });
          dataTR.append("<td>&nbsp;</td>");
          ...
          dataTR.append(
            '<td><span class="locTD">' +
              Object.keys(serialSheetsObj[keys[b] + "_byLoc"][c])[0] +
              " </span>" +
              '<a href="/checking?sheet=' +
              keys[b] +
              "&loc=" +
          ...
          dataTR.append(
            "<td>" +
              serialSheetsObj[keys[b] + "_byLoc"][c][locName + "Check"] +
              "/" +
              serialSheetsObj[keys[b] + "_byLoc"][c][locName + "All"] +
              "</td>"
          );
          dataTR.append(
            '<td><a class="collapseBtn" disabled style="color: grey;"><i class="bi bi-chevron-down"></i></a></td>'
          ...
          var isnCollapseDiv = $("<div>", {
            class: "collapse p-0 m-0",
            id: "locData" + keys[b].substring(0, keys[b].length - 9) + "_" + c,
            "aria-expanded": "false",
          });
          var isnTable = $("<table>", {
            class:
              "table table-success table-hover table-responsive align-items-center m-0 p-0 isnTable",
          });
          var isnthead = $("<thead>", {});
          var tr0 = $("<tr>", {
          ...
          tr0.append(
            '<th class="col col-2">' + Lang.get("checkInvLang.client") + "</th>"
          );
          tr0.append(
            '<th class="col col-2">' +
          ...
          tr0.append(
            '<th class="col col-2">' +
              Lang.get("checkInvLang.updated_by") +
              "</th>"
          );
          tr0.append(
            '<th class="col col-3">' +
              Lang.get("checkInvLang.updated_at") +
              "</th>"
          );
          tr0.append("<th>&nbsp;</th>");
          isnthead.append(tr0);
          ...
          for (
            let n = 0;
            n < serialSheetsObj[keys[b] + "_byLoc"][c][locName].length;
            n++
          ) {
            // loop thru isn under the cth byLoc arrays
            var isnnTR = $(
              '<tr class="align-items-center isnRows" style="vertical-align: middle;">',
              {}
            );
            isnnTR.append("<td>" + (n + 1) + "." + "</td>");
            isnnTR.append(
              '<td><span class="isnTD">' +
                serialSheetsObj[keys[b] + "_byLoc"][c][locName][n].料號 +
                '</span><hr class="m-0 p-0">' +
                serialSheetsObj[keys[b] + "_byLoc"][c][locName][n].品名 +
                "</td>"
            );
            isnnTR.append(
              "<td>" +
                serialSheetsObj[keys[b] + "_byLoc"][c][locName][n].客戶別 +
                "</td>"
            );
            if (
              serialSheetsObj[keys[b] + "_byLoc"][c][locName][n].盤點 === "" ||
              serialSheetsObj[keys[b] + "_byLoc"][c][locName][n].盤點 ===
                null ||
              serialSheetsObj[keys[b] + "_byLoc"][c][locName][n].盤點 === "null"
            ) {
              isnnTR.append(
                '<td class="table-danger table-hover" style="color: red;">' +
                  serialSheetsObj[keys[b] + "_byLoc"][c][locName][n].現有庫存 +
                  '<hr class="m-0 p-0">' +
                  Lang.get("checkInvLang.unknown") +
                  "</td>"
              );
              isnnTR.append(
                "<td>" + Lang.get("checkInvLang.unknown") + "</td>"
              );
              isnnTR.append(
                "<td>" + Lang.get("checkInvLang.unknown") + "</td>"
              );
              ...
              if (
                ...
                isnnTR.append(
                  '<td class="table-danger table-hover" style="color: red;">' +
                    serialSheetsObj[keys[b] + "_byLoc"][c][locName][n]
                      .現有庫存 +
                    '<hr class="m-0 p-0">' +
                    serialSheetsObj[keys[b] + "_byLoc"][c][locName][n].盤點 +
                    "</td>"
              ...
              isnnTR.append(
                "<td>" +
                  serialSheetsObj[keys[b] + "_byLoc"][c][locName][n].姓名 +
                  "</td>"
              );
              isnnTR.append(
                "<td>" +
        ...
        tr.append(
          "<th>" +
            Lang.get("checkInvLang.checked") +
            "/" +
            Lang.get("checkInvLang.all") +
            "</th>"
        );
        tr.append("<th>&nbsp;</th>");
        thead.append(tr);
  ...
  function plainISNTable() {
    ...
    for (let b = 0; b < keys.length; b++) {
      ...
      if (!keys[b].includes("_byLoc")) {
        ...
        tr0.append(
          '<th class="col col-2">' + Lang.get("checkInvLang.loc") + "</th>"
        );
        tr0.append(
          '<th class="col col-2">' +
            Lang.get("checkInvLang.stock") +
        ...
        for (let n = 0; n < serialSheetsObj[keys[b]].length; n++) {
          ...
          isnnTR.append(
            '<td><span class="isnTD">' +
              serialSheetsObj[keys[b]][n].料號 +
              '</span><hr class="m-0 p-0">' +
              serialSheetsObj[keys[b]][n].品名 +
              "</td>"
          );
          isnnTR.append("<td>" + serialSheetsObj[keys[b]][n].客戶別 + "</td>");
          isnnTR.append("<td>" + serialSheetsObj[keys[b]][n].儲位 + "</td>");
          if (
            serialSheetsObj[keys[b]][n].盤點 === "" ||
            serialSheetsObj[keys[b]][n].盤點 === null ||
            serialSheetsObj[keys[b]][n].盤點 === "null"
          ) {
            isnnTR.append(
              '<td class="table-danger table-hover" style="color: red;">' +
                serialSheetsObj[keys[b]][n].現有庫存 +
                '<hr class="m-0 p-0">' +
                Lang.get("checkInvLang.unknown") +
                "</td>"
            );
            ...
            if (
              parseInt(serialSheetsObj[keys[b]][n].現有庫存, 10) !==
              parseInt(serialSheetsObj[keys[b]][n].盤點, 10)
            ) {
              isnnTR.append(
                '<td class="table-danger table-hover" style="color: red;">' +
                  serialSheetsObj[keys[b]][n].現有庫存 +
            ...
            isnnTR.append(
              "<td>" + serialSheetsObj[keys[b]][n].updated_at + "</td>"
            );
          } // else
          isnnTR.append(
            "<td>" +
  ...
  $("#DateRangeString").on("change", function () {
    // trigger ajax post whenever time range is set
    ...
    $.ajax({
        ...
        for (let a = 0; a < myObjs.length; a++) {
          if (serialSheetsObj.hasOwnProperty(myObjs[a].單號)) {
            ...
            for (let c = 0; c < serialSheetsObj[myObjs[a].單號].length; ) {
              ...
              if (
                tempObj["料號"] <= serialSheetsObj[myObjs[a].單號][c]["料號"]
              ) {
              ...
              if (c >= serialSheetsObj[myObjs[a].單號].length) {
                // the current isn is the largest
            ...
            for (
              let z = 0;
              z < serialSheetsObj[myObjs[a].單號 + "_byLoc"].length;

            ) {
              if (
                tempObj["儲位"] ===
                Object.keys(serialSheetsObj[myObjs[a].單號 + "_byLoc"][z])[0]
              ) {
                // Loc already existed
                serialSheetsObj[myObjs[a].單號 + "_byLoc"][z][
                  tempObj["儲位"]
                ].push(tempObj);
                if (tempObj["盤點"] === null) {
                  serialSheetsObj[myObjs[a].單號 + "_byLoc"][z][
              ...
              if (z >= serialSheetsObj[myObjs[a].單號 + "_byLoc"].length) {
                // the loc is a new one
                var tempObjLoc = {};
      ...
      error: function (err) {
        if (err.status === 422) {
          ...
          // else if error : No results Found !
