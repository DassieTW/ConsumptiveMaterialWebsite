...
$(document).ready(function () {
    ...
    $("#inp").on('submit', function (e) {
        ...
        $.ajax({
            ...
            data: { texBox: $temp, isIsn: $isIsn, tablename: tablename, isDetailed: "false" },
    ...
    (function () { // starting show on document ready
        ...
        if (sheetIDFromResultPage !== false) {
            ...
            $.ajax({
                type: "post",
                url: "/checking/checkInentdbSearch",
                data: { tablename: sheetIDFromResultPage, isn: isnFromResultPage, loc: locFromResultPage, isDetailed: "true" },
                dataType: 'json', // expected respose datatype from server
                success: function (response) {
                    // console.log( response.data ) ; // test
                    var myObjs = JSON.parse(JSON.stringify(response.data));
