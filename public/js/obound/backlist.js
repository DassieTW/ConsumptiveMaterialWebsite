...
function addBtn(index) {
    ...
    rowbackreason.innerHTML = "<span id=" + "backreason" + count + ">" + $("#backreason" + index).text() + "</span>";
    ...
    rowstatus.innerHTML = '<select id="status' + count + '"' + 'class = "form-select form-select-lg"' + 'style="width: 150px"' + "required>";

    let rowbound = document.createElement("td");
    rowbound.innerHTML = '<select id="bound' + count + '"' + 'class = "form-select form-select-lg"' + 'style="width: 150px"' + "required>";
...
$(document).ready(function () {
    ...
    $("#backlist").on("submit", function (e) {
        ...
        if (check1 == -1) {
            alert(Lang.get("oboundpageLang.noreceivepeople"));
            $("#pickpeople").addClass("is-invalid");
            return false;
        }

        if (check2 == -1) {
            alert(Lang.get("oboundpageLang.nobackpeople"));
        ...
        for (let i = 0; i < count; i++) {
            if (amount[i] != advance[i] && reason[i] == "") {
                ...
                row = i + 1;
                $("#reasonerrrow").empty().append(Lang.get("oboundpageLang.row") + " " + row);
        ...
        $.ajax({
            ...
            success: function (data) {
                var mess =
                    Lang.get("oboundpageLang.total") + " : " + data.record + ' ' + Lang.get("oboundpageLang.record") +
                    ' ' + Lang.get("oboundpageLang.outbackok") + " : " + data.list;
                alert(mess);
