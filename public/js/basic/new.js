...
function appenSVg(index) {
  ...
  $("#deleteBtn" + index).on("click", function (e) {
    ...
    notyf.success({
      message:
        Lang.get("basicInfoLang.delete") + Lang.get("basicInfoLang.success"),
...
$(function () {
  ...
  $("#newmaterial").on("submit", function (e) {
    ...
    for (const [key, value] of Object.entries(dict)) {
      if ((key !== "safe" && value === "") || value === null) {
        ...
        if (month === "否" && safe === "") {
          ...
          document.getElementById("safe").focus();
          return false;
        }
      }
    }
    $.ajax({
      ...
      success: function (data) {
        ...
        notyf.success({
          message:
            Lang.get("basicInfoLang.add") + Lang.get("basicInfoLang.success"),
          duration: 3000, //miliseconds, use 0 for infinite duration
          ripple: true,
          dismissible: true,
          position: {
  ...
  $("#materialadd").on("submit", function (e) {
    ...
    for (let i = 0; i < sessionStorage.getItem("materialcount"); i++) {
      ...
      if (belong[i] === "Unit consumption" || belong[i] === "单耗")
        belong[i] = "單耗";
