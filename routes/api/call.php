...
// get Yearly 請購vs實際領用Diff
Route::post('/getYearlyDiff', 'api\AlertController@showYearlyDiff');
...
Route::post('/safesearch', function (Request $request) {
    ...
    for ($a = 0; $a < $count; $a++) {
        for ($i = $a; $i + 1 < $count; $i++) {
            if ((isset($datas[$a])) && (isset($datas[$i + 1]))) {
                if ($datas[$a]->料號90 === $datas[$i + 1]->料號90 && $datas[$a]->料號 === $datas[$i + 1]->料號) {
    ...
    $datas1 = DB::table('月請購_站位')
        ->join('MPS', function ($join) {
            $join->on('MPS.料號', '=', '月請購_站位.料號');
                // ->on('MPS.料號90', '=', '月請購_站位.料號90')
        })
        ->join('consumptive_material', function ($join) {
...
Route::post('/daysearch', function (Request $request) {
    ...
    if ($inboundnogood) {
        ...
        $datas1 = $datas1->select('月請購_單耗.料號90', '月請購_單耗.料號', DB::raw('SUM(MPS.下月MPS * 月請購_單耗.單耗 * consumptive_material.LT / MPS.下月生產天數) as 安全庫存'))
            ->groupBy('月請購_單耗.料號', '月請購_單耗.料號90')->get();
        ...
        for ($i = 0; $i < count($datas); $i++) {
            ...
            for ($j = 0; $j < count($datas1); $j++) {
                if ($datas[$i]->料號 === $datas1[$j]->料號 && $datas[$i]->料號90 === $datas1[$j]->料號90) {
    ...
    //normal inventory
        ...
        $datas2 = $datas2->select('月請購_站位.料號', DB::raw('SUM(consumptive_material.LT * 月請購_站位.下月站位人數 * 月請購_站位.下月開線數
                    * 月請購_站位.下月開班數 * 月請購_站位.下月每人每日需求量 * 月請購_站位.下月每日更換頻率 / consumptive_material.MPQ) as 安全庫存'))
            ->groupBy('月請購_站位.料號')->get();
    ...
    if ($inboundmonth) {
        ...
        $datas1 = $datas1->select('月請購_單耗.料號90', '月請購_單耗.料號', DB::raw('SUM(MPS.本月MPS * 月請購_單耗.單耗) as 月使用量'))
            ->groupBy('月請購_單耗.料號', '月請購_單耗.料號90')->get();
