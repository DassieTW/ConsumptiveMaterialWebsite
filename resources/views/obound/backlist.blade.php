    ...
    <div class="card">
        ...
        <div class="card-body">
                        ...
                        <tbody id="backlistbody">
                            ...
                            @foreach ($data as $data)
                                <tr class="isnRows">
                                    <?php
                                    $showstock = '';
                                    $nowstock = DB::table('O庫inventory')
                                        ->where('料號', $data->料號)
                                        ->where('客戶別', $data->客戶別)
                                        ->where('現有庫存', '>', 0)
                                        ->pluck('現有庫存')
                                        ->toArray();
                                    $nowbound = DB::table('O庫inventory')
                                        ->where('料號', $data->料號)
                                        ->where('客戶別', $data->客戶別)
                                        ->where('現有庫存', '>', 0)
                                        ->pluck('庫別')
                                    ...
                                    <td>
                                        <div class="tooltip1">
                                            <input class="form-control amount" style="width:100px" type="number"
                                                ...
                                                value="{{ $data->實際退回數量 }}" min="1" value="1">
                                    ...
                                    <td>
                                        <select style="width: 150px" class="form-select form-select-lg"
                                            name="bound{{ $loop->index }}" id="bound{{ $loop->index }}">
            ...
            <button class="btn btn-lg btn-primary"
                onclick="location.href='{{ route('obound.backlistpage') }}'">{!! __('oboundpageLang.return') !!}</button>
        </div>
    </div>
@endsection
