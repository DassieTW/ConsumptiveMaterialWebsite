            ...
            <div class="card-body">
                        ...
                        <form action="{{ route('outbound.download') }}" method="POST">
                            ...
                            <tr id = "require">
                                ...
                                <th><input type = "hidden" id = "title3" name = "title3" value = "退回原因">{!! __('outboundpageLang.backreason') !!}</th>
                                ...
                                <th><input type = "hidden" id = "title9" name = "title9" value = "預退數量">{!! __('outboundpageLang.backamount') !!}</th>
                                <th><input type = "hidden" id = "title10" name = "title10" value = "實際退回數量">{!! __('outboundpageLang.realbackamount') !!}</th>
                                <th><input type = "hidden" id = "title11" name = "title11" value = "備註">{!! __('outboundpageLang.mark') !!}</th>
                                <th><input type = "hidden" id = "title12" name = "title12" value = "實退差異原因">{!! __('outboundpageLang.backdiffreason') !!}</th>
                                <th><input type = "hidden" id = "title13" name = "title13" value = "儲位">{!! __('outboundpageLang.loc') !!}</th>
                                <th><input type = "hidden" id = "title14" name = "title14" value = "收料人員">{!! __('outboundpageLang.receivepeople') !!}</th>
                                <th><input type = "hidden" id = "title15" name = "title15" value = "收料人員工號">{!! __('outboundpageLang.receivepeoplenum') !!}</th>
                                <th><input type = "hidden" id = "title16" name = "title16" value = "退料人員">{!! __('outboundpageLang.backpeople') !!}</th>
                                <th><input type = "hidden" id = "title17" name = "title17" value = "退料人員工號">{!! __('outboundpageLang.backpeoplenum') !!}</th>
                                <th><input type = "hidden" id = "title18" name = "title18" value = "退料單號">{!! __('outboundpageLang.backlistnum') !!}</th>
                                <th><input type = "hidden" id = "title19" name = "title19" value = "開單時間">{!! __('outboundpageLang.opentime') !!}</th>
                                <th><input type = "hidden" id = "title20" name = "title20" value = "入庫時間">{!! __('outboundpageLang.inboundtime') !!}</th>
                                <th><input type = "hidden" id = "title20" name = "title20" value = "功能狀況">{!! __('outboundpageLang.status') !!}</th>
                                ...
                                <tr id = "{{$data->退料單號}}">
                                    ...
                                    <td><input type = "hidden" id = "data9{{$loop->index}}" name = "data9{{$loop->index}}" value = "{{$data->預退數量}}">{{$data->預退數量}}</td>
                                    <td><input type = "hidden" id = "data10{{$loop->index}}" name = "data10{{$loop->index}}" value = "{{$data->實際退回數量}}">{{$data->實際退回數量}}</td>
                                    <td><input type = "hidden" id = "data11{{$loop->index}}" name = "data11{{$loop->index}}" value = "{{$data->備註}}">{{$data->備註}}</td>
                                    <td><input type = "hidden" id = "data12{{$loop->index}}" name = "data12{{$loop->index}}" value = "{{$data->實退差異原因}}">{{$data->實退差異原因}}</td>
                                    <td><input type = "hidden" id = "data13{{$loop->index}}" name = "data13{{$loop->index}}" value = "{{$data->儲位}}">{{$data->儲位}}</td>
                                    <td><input type = "hidden" id = "data14{{$loop->index}}" name = "data14{{$loop->index}}" value = "{{$data->收料人員}}">{{$data->收料人員}}</td>
                                    <td><input type = "hidden" id = "data15{{$loop->index}}" name = "data15{{$loop->index}}" value = "{{$data->收料人員工號}}">{{$data->收料人員工號}}</td>
                                    <td><input type = "hidden" id = "data16{{$loop->index}}" name = "data16{{$loop->index}}" value = "{{$data->退料人員}}">{{$data->退料人員}}</td>
                                    <td><input type = "hidden" id = "data17{{$loop->index}}" name = "data17{{$loop->index}}" value = "{{$data->退料人員工號}}">{{$data->退料人員工號}}</td>
                                    <td><input type = "hidden" id = "data18{{$loop->index}}" name = "data18{{$loop->index}}" value = "{{$data->退料單號}}">{{$data->退料單號}}</td>
                                    <td><input type = "hidden" id = "data19{{$loop->index}}" name = "data19{{$loop->index}}" value = "{{$data->開單時間}}">{{$data->開單時間}}</td>
                                    <td><input type = "hidden" id = "data20{{$loop->index}}" name = "data20{{$loop->index}}" value = "{{$data->入庫時間}}">{{$data->入庫時間}}</td>
                        ...
                        <br>
                <button class="btn btn-lg btn-primary" onclick="location.href='{{route('outbound.backrecord')}}'">{!! __('outboundpageLang.return') !!}</button>
            </div>
        </div>
</html>
@endsection
