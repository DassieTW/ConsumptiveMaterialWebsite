                        ...
                        <thead>
                            <tr id="require">
                                ...
                                <th><input type="hidden" id="title0" name="title0"
                                        value="調撥單號">{!! __('bupagelang.dblist') !!}</th>
                                <th><input type="hidden" id="title1" name="title1"
                                        value="狀態">{!! __('bupagelang.status') !!}</th>
                                <th><input type="hidden" id="title2" name="title2"
                                        value="撥出廠區">{!! __('bupagelang.outfactory') !!}</th>
                                ...
                                <th><input type="hidden" id="title12" name="title12"
                                        value="撥出數量">{!! __('bupagelang.transamount') !!}
                                </th>
                                <th><input type="hidden" id="title13" name="title13"
                                        value="接收數量">{!! __('bupagelang.receiveamount') !!}
                                </th>
                                <th><input type="hidden" id="title14" name="title14"
                                        value="開單時間">{!! __('bupagelang.opentime') !!}
                                </th>
                                <th><input type="hidden" id="title15" name="title15"
                                        value="出庫時間">{!! __('bupagelang.outboundtime') !!}
                                </th>
                                <th><input type="hidden" id="title16" name="title16"
                                        value="入庫時間">{!! __('bupagelang.inboundtime') !!}
                                </th>
                            ...
                            @foreach ($data as $data)
                                <tr id="list{{ $loop->index }}" class="isnRows">
                                    ...
                                    <td><input type="hidden" id="datan{{ $loop->index }}"
                                            ...
                                            value="{{ $data->接收數量 }}">{{ $data->接收數量 }}</td>
                                    <td><input type="hidden" id="datao{{ $loop->index }}"
                                            name="datao{{ $loop->index }}"
                                            value="{{ $data->開單時間 }}">{{ $data->開單時間 }}</td>
                                    <td><input type="hidden" id="datap{{ $loop->index }}"
                                            name="datap{{ $loop->index }}"
                                            value="{{ $data->出庫時間 }}">{{ $data->出庫時間 }}</td>
                                    <td><input type="hidden" id="dataq{{ $loop->index }}"
                                            name="dataq{{ $loop->index }}"
