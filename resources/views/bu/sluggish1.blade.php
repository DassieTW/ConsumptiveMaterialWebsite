            ...
            <form id="sluggish" method="POST">
                ...
                <table class="table" id="test">
                    ...
                    <tbody>
                        ...
                        --}}
                        @if(isset($table))
                        @for($i = 1 ; $i < 14 ; $i++) @foreach($test[$i] as $data) <?php
                            $maxtime=date_create(date('Y-m-d',strtotime($data->
                            inventory最後更新時間)));
                            ...
                            $buytime[$key][0] = $value;
                            $buytime[$key][1] = DB::table('consumptive_material')->where('料號',
                            $data->料號)->value('發料部門');
                            $buytime1[$key][0] = $value;
                            $buytime1[$key][1] = DB::table('consumptive_material')->where('料號',
                            $data->料號)->value('發料部門');
                            ...
                            $buytimeco[$key][1] = DB::table('consumptive_material')->where('料號',
                            $data->料號)->value('發料部門');
                            $buytimeco1[$key][0] = $value;
                            $buytimeco1[$key][1] = DB::table('consumptive_material')->where('料號',
                            $data->料號)->value('發料部門');
                            ...
                            <tr>
                                ...
                                <td><input type="hidden" id="dataa{{$i}}{{$loop->index}}"
                                        name="dataa{{$i}}{{$loop->index}}" value="{{$database[$i]}}">{{$database[$i]}}
                                </td>
                                <td><input type="hidden" id="datab{{$i}}{{$loop->index}}"
                                        name="datab{{$i}}{{$loop->index}}" value={{$data->料號}}>{{$data->料號}}</td>
                                <td><input type="hidden" id="datac{{$i}}{{$loop->index}}"
                                        name="datac{{$i}}{{$loop->index}}" value={{$data->品名}}>{{$data->品名}}</td>
                                <td><input type="hidden" id="datad{{$i}}{{$loop->index}}"
                                        name="datad{{$i}}{{$loop->index}}" value={{$data->規格}}>{{$data->規格}}</td>
                                <td><input type="hidden" id="datae{{$i}}{{$loop->index}}"
                                        name="datae{{$i}}{{$loop->index}}" value={{$data->單位}}>{{$data->單位}}</td>
                                <td><input type="hidden" id="dataf{{$i}}{{$loop->index}}"
                                        name="dataf{{$i}}{{$loop->index}}" value={{$stayday}}>{{$stayday}}</td>
                                ...
                                <td><input type="number" id="datah{{$i}}{{$loop->index}}"
                                        class="form-control form-control-lg" name="data7{{$i}}{{$loop->index}}"
                                        value="1" style="width:90px;" min="0"></td>
                                <td id="datai{{$i}}{{$loop->index}}" name="datai{{$i}}{{$loop->index}}">
                                    ...
                                    <span style="white-space: pre-line">{!! __('bupagelang.factory') !!} :
                                        ...
                                        {!! __('bupagelang.buytime')!!} : {{$buytime[2]}}</span><br>
                                    @endif
                                    @endforeach
                                    ...
                                    <span style="white-space: pre-line">{!! __('bupagelang.factory') !!} :
                                        ...
                                        {!! __('bupagelang.buytime') !!} : {{$buytime1[2]}}</span><br>
                                    @endif
                                    @endforeach
                                </td>
                                <td>
                                    <select class="form-select form-select-lg" id="dataj{{$i}}{{$loop->index}}"
                                        ...
                                        @if($buytime[0] !== 'Consumables management')
                            ...
                            @endfor
                            @else
                            {{-- loop thru every database, except for the default Consumables management, so starting at
                            1
                            --}}
                            @for($i = 1 ; $i < 14 ; $i++) @foreach($test[$i] as $data) <?php
                                $maxtime=date_create(date('Y-m-d',strtotime($data->
                                ...
                                ?> <tr class="isnRows">
                                    ...
                                    <td><input type="number" id="datah{{$i}}{{$loop->index}}"
                                            name="datah{{$i}}{{$loop->index}}" value="1" min="1"
                                            class="form-control formcontrol-lg" style="width:90px;"></td>
                                    <td id="datai{{$i}}{{$loop->index}}" name="datai{{$i}}{{$loop->index}}">
                                        @foreach($buytime as $buytime)
                                        @if( $buytime[2] != null)
                                        <span style="white-space: pre-line">{!! __('bupagelang.factory') !!} :
                                    ...
                                    <td>
                                        <select class="form-select form-select-lg" id="dataj{{$i}}{{$loop->index}}"
                                            name="dataj{{$i}}{{$loop->index}}" style="width: 90px">
                                            <option style="display: none" disabled selected>{!!
                                                __('bupagelang.enterfactory')
                                                !!}</option>
                                            @foreach($buytimeco as $buytime)
                                            @if($buytime[0] !== 'Consumables management')
                                            <option>{{ str_replace(" Consumables management", " ", $buytime[0]) }}
                                            </option>
                                            @endif
                                            @endforeach
                                        </select>
